...
--Query 1 fetch  all  employee name and their dept name they are working to 
--right join  = right join + any additional column in right join 
select e.emp_name,d.dept_name
from employees as e right  join department as d 
on d.DEPT_ID = e.dept_id


--fetch details of all employees their manager their department and project they work on 
...
SELECT * FROM Department
--solution 
select e.emp_name,d.DEPT_NAME, MANAGER_NAME,P.PROJECT_NAME
from employees as e 
left join department as d on e.dept_id = d.DEPT_ID  
INNER JOIN  MANAGER AS M ON E.manager_id = M.MANAGER_ID
LEFT JOIN  PROJECTS AS P ON E.emp_id = P.TEAM_MEMBER_ID 